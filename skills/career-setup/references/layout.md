...
| `fixtures/dart/` | 사업보고서 파서 회귀 픽스처 | `research-role` 파서 수정 | `test_extract.py` — `ADD_COMPANY_FIXTURES`가 이 경로 |
...
| 이름 | 여는 것 | 발급 |
|---|---|---|
| `DART_API_KEY` | `research-role` 뻗기의 DART 각도(`angles.md` 1·2) | https://opendart.fss.or.kr/uss/umt/EgovMberInsertView.do — 개인용 즉시 발급, 무료, 1인 1키 |
...
**로그인은 접근 등급을 올리지 않는다** — 등급과 예외 규칙은 `research-role/references/access.md` #2가 소유한다. 요지: 파일이 있는 호스트는 사용자 계정으로 한 페이지씩 열 수 있고 출처 뒤에 `규칙 예외(auth)`를 적는다. 파일이 없는 호스트는 링크만 남기고 사용자에게 `login`을 안내한다. 목록 순회는 하지 않는다 — 실마리가 가리킨 페이지 1건씩이다.
...
`gh`의 로그인 상태는 `gh` 자신이 `~/.config/gh/`에 둔다 — 데이터 디렉토리에는 파일이 없고, 있는지는 `gh auth status`의 종료 코드로만 본다. 읽는 것은 `extract-repo`다 — 원격 리포를 `cache/repos/<slug>/`에 받고 PR·이슈·리뷰를 가져온다. 로그인이 없으면 공개 리포 클론과 로컬 디렉토리만 된다.
