...
공통 규칙: 호스트당 요청 사이에 간격을 두고, 403·429가 오면 그 호스트는 즉시 중단한다.
...
| 15 | 특허 | 어디에 기술을 걸었나 — 출원 분야 | KIPRIS · 기사 | WebSearch "<법인명> 특허 출원" | C | KIPRIS는 robots `Disallow: /` ①, KIPRIS Plus API는 키·조건 미확인 → 링크만 |
...
- 각도당 대표 자료 최대 5건, 최근 2년 우선. 5번 채용공고만 전체.
...
- DART 묶음이 `dart_extract.py` 종료 2·3을 받으면 원문 XML을 `fixtures/dart/{rcept_no}.xml`로 복사하고 픽스처 README 표에 한 줄 추가한 뒤 "못 찾은 것"에 `실패`로 보고한다. 파서를 손으로 우회하지 않는다 — 수정은 별도 에이전트의 일이다.
